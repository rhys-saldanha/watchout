import pygame, sys, time, random, operator, math
...
#DISPLAYSURF = pygame.display.set_mode((800,00), 0)
...
def main():
    ...
    global scoreMultiplier, lives, myfontbig, movei, acel, movetime, cheat
    ...
    for rock in golden:
        chance = random.randint(1, 1000000000) 
        if chance == 69:
            rockx = random.randint(-200, screenx+200)
            rocky = random.randint(-200, screeny+200)
            rock = [rockx, rocky]
            if len(golden) < 1:
                golden.append(rock)
    ...
    score = myfontbig.render('Score: ' + (str(scoreTimer.GetPast())), 1, GREEN)
    DISPLAYSURF.blit(score, (20, 20))
    
    livesDisplay = myfontbig.render('Lives: %s' % lives, 1, GREEN)
    DISPLAYSURF.blit(livesDisplay, (centrex - 70, 20))
    
    multiplier = myfontbig.render('Gold Rocks: ' + str(scoreMultiplier - 2), 1, GREEN)
    DISPLAYSURF.blit(multiplier, (screenx - 300, 20))
